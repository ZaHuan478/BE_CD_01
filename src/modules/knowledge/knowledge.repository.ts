...
import type { CreateDocumentBody, CreateGuidanceBody, CreateTermBody, KnowledgeQuery } from './knowledge.schemas.js'
...
export class KnowledgeRepository {
  ...
  async listDocuments(filters: KnowledgeQuery) {
    ...
    if (filters.sopId) {
      conditions.push('EXISTS (SELECT 1 FROM dbo.DocumentLink filterLink WHERE filterLink.DocumentId = documentRow.DocumentId AND filterLink.SopId = @sopId)')
    ...
    const rows = await this.database.query<{
      ...
      SELECT documentRow.DocumentId, documentRow.DocumentCode, documentRow.Title,
             ...
             (SELECT STRING_AGG(CAST(link.SopId AS NVARCHAR(MAX)), '|') FROM dbo.DocumentLink link WHERE link.DocumentId = documentRow.DocumentId) AS SopIds,
             (SELECT STRING_AGG(CAST(moduleLink.ModuleId AS NVARCHAR(MAX)), '|')
                FROM dbo.DocumentLink link
                INNER JOIN dbo.SopModule moduleLink ON moduleLink.SopId = link.SopId
                WHERE link.DocumentId = documentRow.DocumentId) AS ModuleIds
      FROM dbo.Document documentRow
      ${where}
  ...
  async createDocument(body: CreateDocumentBody, actorAccountId: string) {
    ...
    await this.database.transaction(async (runner) => {
        ...
        const valid = await runner.query<{ SopId: string }>(`
          ...
          WHERE s.SopId = @sopId
            AND (@versionId IS NULL OR EXISTS (
              SELECT 1 FROM dbo.SopVersion v WHERE v.SopVersionId = @versionId AND v.SopId = s.SopId
            ))
            AND (@stepId IS NULL OR EXISTS (
              SELECT 1 FROM dbo.SopStep stepRow
      ...
      await runner.query(`
        INSERT INTO dbo.Document (DocumentId, DocumentCode, Title, AssetUrl, MediaType, Checksum, MetadataJson)
  ...
  async listTerms(filters: KnowledgeQuery) {
    ...
    const rows = await this.database.query<{
      ...
      FROM dbo.GlossaryTerm WHERE ${conditions.join(' AND ')} ORDER BY Term
    `, parameters)
    return rows.map((row) => ({
  ...
  async createTerm(body: CreateTermBody, actorAccountId: string) {
    ...
    await this.database.transaction(async (runner) => {
      await runner.query(`
        INSERT INTO dbo.GlossaryTerm (GlossaryTermId, Term, Definition, AliasesJson, ModuleId, Status)
  ...
  async listGuidance(filters: KnowledgeQuery) {
    ...
    const rows = await this.database.query<{
      ...
      FROM dbo.GuidanceArticle WHERE ${conditions.join(' AND ')} ORDER BY Title
    `, parameters)
    return rows.map((row) => ({
  ...
  async createGuidance(body: CreateGuidanceBody, actorAccountId: string) {
    ...
    await this.database.transaction(async (runner) => {
      await runner.query(`
        INSERT INTO dbo.GuidanceArticle (
