...
export class SopRepository {
  ...
  async list(filters: { moduleId?: string; search?: string; includeDrafts: boolean }) {
    ...
    if (filters.moduleId) {
      conditions.push('EXISTS (SELECT 1 FROM dbo.SopModule filterLink WHERE filterLink.SopId = s.SopId AND filterLink.ModuleId = @moduleId)')
    ...
    const rows = await this.database.query<SopListRow>(`
      SELECT s.SopId, s.SopCode, s.Title, s.Category,
             ...
             (SELECT STRING_AGG(CAST(link.ModuleId AS NVARCHAR(MAX)), '|') FROM dbo.SopModule link WHERE link.SopId = s.SopId) AS ModuleIds
      FROM dbo.Sop s
      OUTER APPLY (
        SELECT TOP 1 v.SopVersionId, v.VersionNumber, v.PublicationStatus, v.UpdatedAt
        FROM dbo.SopVersion v
  ...
  async getAccessContext(sopId: string): Promise<{ id: string; title: string; moduleIds: string[] }> {
    const rows = await this.database.query<SopAccessRow>(`
      ...
      FROM dbo.Sop s
      LEFT JOIN dbo.SopModule link ON link.SopId = s.SopId
      WHERE s.SopId = @sopId
    `, { sopId })
  ...
  async getDetail(sopId: string, versionId?: string) {
    ...
    const versions = await this.database.query<VersionRow>(`
      SELECT v.SopVersionId, v.SopId, v.VersionNumber, v.PublicationStatus,
             ...
             v.UpdatedAt, v.PublishedAt, CONVERT(VARCHAR(18), v.RowVersion, 1) AS RowVersion
      FROM dbo.Sop s
      INNER JOIN dbo.SopVersion v ON v.SopId = s.SopId
    ...
    const [sopRows, steps, artifacts, transitions, moduleLinks, history] = await Promise.all([
      this.database.query<{ SopCode: string; Title: string; Category: string | null }>(`
        SELECT SopCode, Title, Category FROM dbo.Sop WHERE SopId = @sopId
      `, { sopId }),
      this.database.query<StepRow>(`
        ...
        FROM dbo.SopStep WHERE SopVersionId = @versionId ORDER BY SortOrder, StepCode
      ...
      this.database.query<ArtifactRow>(`
        ...
        FROM dbo.StepArtifact a
        INNER JOIN dbo.SopStep stepRow ON stepRow.SopStepId = a.SopStepId
      ...
      this.database.query<TransitionRow>(`
        SELECT SopTransitionId, FromStepId, ToStepId, TransitionKind,
               ConditionText, BranchLabel, TargetSopId, SortOrder
        FROM dbo.SopTransition WHERE SopVersionId = @versionId ORDER BY SortOrder
      `, { versionId: version.SopVersionId }),
      this.database.query<ModuleLinkRow>(`
        SELECT ModuleId, IsPrimary, RelationType FROM dbo.SopModule WHERE SopId = @sopId
      `, { sopId }),
      this.database.query<VersionSummaryRow>(`
        SELECT SopVersionId, VersionNumber, PublicationStatus, CreatedAt, UpdatedAt,
               PublishedAt, CONVERT(VARCHAR(18), RowVersion, 1) AS RowVersion
        FROM dbo.SopVersion WHERE SopId = @sopId ORDER BY VersionNumber DESC
      `, { sopId })
    ])
    const sop = sopRows[0]
  ...
  async create(body: CreateSopBody, actorAccountId: string): Promise<{ sopId: string; versionId: string }> {
    ...
    await this.database.transaction(async (runner) => {
      ...
      const existing = await runner.query<{ ModuleId: string }>(`
        SELECT ModuleId FROM dbo.HrModule WHERE ModuleId IN (${placeholders})
      `, params)
      if (existing.length !== uniqueModuleIds.length) {
        throw conflict('MODULE_NOT_FOUND', 'One or more moduleIds do not exist')
      }
      await runner.query(`
        INSERT INTO dbo.Sop (SopId, SopCode, Title, Category, CreatedBy)
      ...
      for (const moduleId of uniqueModuleIds) {
        await runner.query(`
          INSERT INTO dbo.SopModule (SopId, ModuleId, IsPrimary, RelationType)
  ...
  async updateSop(sopId: string, body: UpdateSopBody, actorAccountId: string): Promise<void> {
    ...
    await this.database.transaction(async (runner) => {
      const current = await runner.query<{ Title: string; Category: string | null }>(`
        SELECT Title, Category FROM dbo.Sop WITH (UPDLOCK, ROWLOCK) WHERE SopId = @sopId
      `, { sopId })
      if (!current[0]) throw notFound('SOP', sopId)
      await runner.query(`
        UPDATE dbo.Sop
        SET Title = @title, Category = @category, UpdatedAt = SYSUTCDATETIME()
        WHERE SopId = @sopId
      `, {
        sopId,
      ...
      if (body.moduleIds && body.primaryModuleId) {
        ...
        await runner.query('DELETE FROM dbo.SopModule WHERE SopId = @sopId', { sopId })
  ...
  async replaceVersion(versionId: string, body: ReplaceSopVersionBody, actorAccountId: string): Promise<string> {
    return this.database.transaction(async (runner) => {
      const currentRows = await runner.query<{ PublicationStatus: string; RowVersion: string }>(`
        SELECT PublicationStatus, CONVERT(VARCHAR(18), RowVersion, 1) AS RowVersion
        FROM dbo.SopVersion WITH (UPDLOCK, ROWLOCK)
      ...
      const updated = await runner.query<{ RowVersion: string }>(`
        UPDATE dbo.SopVersion
        SET Definition = @definition, Purpose = @purpose, Scope = @scope,
            ChangeLog = @changeLog, UpdatedAt = SYSUTCDATETIME()
        OUTPUT CONVERT(VARCHAR(18), inserted.RowVersion, 1) AS RowVersion
        WHERE SopVersionId = @versionId
          AND RowVersion = CONVERT(BINARY(8), @expectedRowVersion, 1)
      `, {
        versionId,
      ...
      if (!updated[0]) throw conflict('VERSION_CONFLICT', 'This version was changed by another request')
      await runner.query('DELETE FROM dbo.SopTransition WHERE SopVersionId = @versionId', { versionId })
      await runner.query(`
        DELETE artifact FROM dbo.StepArtifact artifact
        INNER JOIN dbo.SopStep stepRow ON stepRow.SopStepId = artifact.SopStepId
        WHERE stepRow.SopVersionId = @versionId
      `, { versionId })
      await runner.query('DELETE FROM dbo.SopStep WHERE SopVersionId = @versionId', { versionId })
      await insertContent(runner, versionId, body)
      await runner.query(`
        INSERT INTO dbo.AuditLog (EntityType, EntityId, Action, ActorAccountId, AfterJson)
  ...
  async createDraft(sopId: string, actorAccountId: string): Promise<string> {
    const sourceRows = await this.database.query<{ SopVersionId: string }>(`
      SELECT TOP 1 SopVersionId FROM dbo.SopVersion
      WHERE SopId = @sopId
    ...
    await this.database.transaction(async (runner) => {
      const nextRows = await runner.query<{ VersionNumber: number }>(`
        SELECT ISNULL(MAX(VersionNumber), 0) + 1 AS VersionNumber
        FROM dbo.SopVersion WITH (UPDLOCK, HOLDLOCK) WHERE SopId = @sopId
      `, { sopId })
      const nextVersion = nextRows[0]
      if (!nextVersion) throw conflict('VERSION_NUMBER_FAILED', 'Could not allocate a version number')
      await runner.query(`
        INSERT INTO dbo.SopVersion (
          SopVersionId, SopId, VersionNumber, PublicationStatus,
          Definition, Purpose, Scope, ChangeLog, CreatedBy
  ...
  async submitVersion(versionId: string, actorAccountId: string): Promise<void> {
    const updated = await this.database.query<{ SopVersionId: string }>(`
      UPDATE dbo.SopVersion
      SET PublicationStatus = 'in_review', UpdatedAt = SYSUTCDATETIME()
      OUTPUT inserted.SopVersionId
      WHERE SopVersionId = @versionId AND PublicationStatus IN ('draft', 'rejected')
    `, { versionId })
    if (!updated[0]) throw conflict('VERSION_NOT_SUBMITTABLE', 'Only draft or rejected versions can be submitted')
    await this.database.query(`
      INSERT INTO dbo.AuditLog (EntityType, EntityId, Action, ActorAccountId)
  ...
  async publishVersion(versionId: string, actorAccountId: string): Promise<void> {
    await this.database.transaction(async (runner) => {
      const versions = await runner.query<{ SopId: string; PublicationStatus: string }>(`
        SELECT SopId, PublicationStatus FROM dbo.SopVersion WITH (UPDLOCK, ROWLOCK)
        WHERE SopVersionId = @versionId
      `, { versionId })
      const version = versions[0]
      ...
      await runner.query(`
        UPDATE dbo.SopVersion
        SET PublicationStatus = 'archived', ValidTo = SYSUTCDATETIME(), UpdatedAt = SYSUTCDATETIME()
        WHERE SopId = @sopId AND PublicationStatus = 'published'
      `, { sopId: version.SopId })
      await runner.query(`
        DECLARE @snapshot NVARCHAR(MAX);
        SELECT @snapshot = (
          SELECT s.SopId AS sopId, s.SopCode AS sopCode, s.Title AS title, s.Category AS category,
                 v.SopVersionId AS versionId, v.VersionNumber AS versionNumber,
                 v.Definition AS definition, v.Purpose AS purpose, v.Scope AS scope,
                 JSON_QUERY((
                   SELECT stepRow.SopStepId AS id, stepRow.StableKey AS stableKey,
                          stepRow.StepCode AS code, stepRow.Title AS title,
                          stepRow.Objective AS objective, stepRow.Description AS description,
                          stepRow.Actor AS actor, stepRow.Location AS location,
                          stepRow.Timing AS timing, stepRow.NodeKind AS nodeKind,
                          stepRow.TypeCode AS typeCode, stepRow.SortOrder AS sortOrder,
                          JSON_QUERY(stepRow.ChecklistJson) AS checklist,
                          JSON_QUERY((
                            SELECT artifact.StepArtifactId AS id, artifact.Direction AS direction,
                                   artifact.Name AS name, artifact.Description AS description,
                                   artifact.IsRequired AS required, artifact.SortOrder AS sortOrder,
                                   JSON_QUERY(artifact.MetadataJson) AS metadata
                            FROM dbo.StepArtifact artifact
                            WHERE artifact.SopStepId = stepRow.SopStepId
                            ORDER BY artifact.Direction, artifact.SortOrder
                            FOR JSON PATH
                          )) AS artifacts
                   FROM dbo.SopStep stepRow
                   WHERE stepRow.SopVersionId = v.SopVersionId
                   ORDER BY stepRow.SortOrder, stepRow.StepCode
                   FOR JSON PATH
                 )) AS steps,
                 JSON_QUERY((
                   SELECT transitionRow.SopTransitionId AS id,
                          transitionRow.FromStepId AS fromStepId,
                          transitionRow.ToStepId AS toStepId,
                          transitionRow.TransitionKind AS kind,
                          transitionRow.ConditionText AS condition,
                          transitionRow.BranchLabel AS branchLabel,
                          transitionRow.TargetSopId AS targetSopId,
                          transitionRow.SortOrder AS sortOrder
                   FROM dbo.SopTransition transitionRow
                   WHERE transitionRow.SopVersionId = v.SopVersionId
                   ORDER BY transitionRow.SortOrder
                   FOR JSON PATH
                 )) AS transitions
          FROM dbo.SopVersion v
          INNER JOIN dbo.Sop s ON s.SopId = v.SopId
          WHERE v.SopVersionId = @versionId
          FOR JSON PATH, WITHOUT_ARRAY_WRAPPER
        );

        UPDATE dbo.SopVersion
        SET PublicationStatus = 'published', ReviewedBy = @actorAccountId,
            PublishedBy = @actorAccountId, PublishedAt = SYSUTCDATETIME(),
            ValidFrom = SYSUTCDATETIME(), ValidTo = NULL, UpdatedAt = SYSUTCDATETIME(),
            ContentSnapshotJson = @snapshot
        WHERE SopVersionId = @versionId
      `, { versionId, actorAccountId })
      await runner.query(`
        UPDATE dbo.Sop SET CurrentPublishedVersionId = @versionId, UpdatedAt = SYSUTCDATETIME()
        WHERE SopId = @sopId
  ...
  async rejectVersion(versionId: string, reason: string, actorAccountId: string): Promise<void> {
    await this.database.transaction(async (runner) => {
      const updated = await runner.query<{ SopVersionId: string }>(`
        UPDATE dbo.SopVersion
  ...
  async getSopIdForVersion(versionId: string): Promise<string> {
    const rows = await this.database.query<{ SopId: string }>(`
      SELECT SopId FROM dbo.SopVersion WHERE SopVersionId = @versionId
    `, { versionId })
